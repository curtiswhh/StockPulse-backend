...
// extended / closed) but adds a /tick-specific concept: a 20-minute "trailing
// grace window" after the 4pm ET close.
//
// Why the grace window: the moments right after the bell are when
// reset-window adjustments land — final prints, late corrections, day-summary
// fields populating in Polygon. We want to catch those for evaluators that
// depend on `todaysChangePerc`, then go quiet until the next session.
//
// All other hours (pre-market < 09:30, after 16:20, weekends): /tick returns
...
const GRACE_END_MIN     = 16 * 60 + 20; // 16:20 ET (close + 20 min)
